...
(defun parse (file)
  (let ((pond (make-list 9 0)))
    (dolist (fish (slurp-numbers file))
      (cl-incf (nth fish pond)))
    pond))

(defun step (pond)
  (let ((deaths (car pond)))
    (dotimes (i (- (length pond) 1))
      (setf (nth i pond)
            (nth (+ i 1) pond)))
    (setf (nth 8 pond) deaths)
    (cl-incf (nth 6 pond) deaths)
    pond))

(defun part1 (file steps)
  (let ((pond (parse file)))
    (dotimes (_ steps)
      (setq pond (step pond)))
    (apply #'+ pond)))
