...
(defun parse (raw-lines)
  (let ((boards '())
        (board '()))
    (dolist (line raw-lines)
      (cond
       ((equal "" line)
        (push (reverse board) boards)
        (setq board '()))
       (t
        (push
         (mapcar #'string-to-number (split-string line))
         board))))
    (reverse (cons (reverse board) boards))))

(defun winner-board (board)
  (or
   (seq-some #'winner-line board)
   (seq-some #'winner-line (transpose board))))

(defun winner-line (line)
  (seq-every-p (lambda (el) (member el drawn)) line))

(defun score (board)
  (*
   (car drawn)
   (apply #'+ (seq-remove (lambda (el) (member el drawn)) (flatten-list board)))))
