...
(defun step (pond)
  (let ((new '()))
    (dolist (fish pond)
      (cl-decf (fish-timer fish))
      (when (= (fish-timer fish) -1)
        (setf (fish-timer fish) 6)
        (push (make-fish :timer 8) new)))
    (append pond new)))

(defun simulate (pond n)
  (princ "initial: ")
  (print-pond pond)
  (princ "\n")
  (dotimes (i n)
    (setq pond (step pond))
    (princ (format "%d (%d): " (+ i 1) (length pond)))
    (print-pond pond)
    (princ "\n")
    )
  pond)
...
(defun part1 (file)
  (length
   (simulate
    (mapcar (lambda (n) (make-fish :timer n)) (slurp-numbers file))
    18)))
...
(defun print-pond (pond)
  (dolist (fish pond)
    (princ (fish-timer fish))
    (princ ",")))
