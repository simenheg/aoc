...
(defun shared-item (line)
  (seq-let (a b) (seq-partition line (/ (length line) 2))
    (car (seq-intersection a b))))
...
(defun part1 (file)
  (sum (mapcar #'pri (mapcar #'shared-item (slurp file)))))
...
(defun shared-item-2 (line)
  (car (seq-intersection
        (nth 0 line)
        (seq-intersection (nth 1 line) (nth 2 line)))))

(defun part2 (file)
  (sum (mapcar #'pri (mapcar #'shared-item-2 (seq-partition (slurp file) 3)))))
