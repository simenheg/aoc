...
;;     The top-middle 5 is visible from the top and right.q
...
;;     The right-middle 3 is visible from the right.  In the bottom
;;     row, the middle 5 is visible, but the 3 and 4 are not.
...
(defun visiblep (m x y)
  (or (visiblep* m x y 1 0)
      (visiblep* m x y -1 0)
      (visiblep* m x y 0 1)
      (visiblep* m x y 0 -1)))
...
(defun part2 (file)
  (let ((m (slurp-matrix file))
    ...
    (dotimes (x (length (car m)))
      (dotimes (y (length m))
        (setq best (max best (score m x y)))))
    best))
