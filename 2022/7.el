...
(cl-defstruct dir
  name parent dirs files)

(cl-defstruct file
  name size)

(defun fill-dir (dir args)
  (dolist (arg args)
    (pcase (split-string arg)
      (`("dir" ,name)
       (push (make-dir :name name :parent dir) (dir-dirs dir)))
      (`(,size ,name)
       (push (make-file :name name :size (string-to-number size))
             (dir-files dir))))))
        ...
        (`("$" "ls")
         (fill-dir
          ...
          (seq-take-while
           ...
           lines)))
        (`("$" "cd" "/") (setq dir root))
        (`("$" "cd" "..") (setq dir (dir-parent dir)))
        (`("$" "cd" ,arg)
         (setq dir (seq-find (lambda (d) (equal arg (dir-name d)))
                             (dir-dirs dir))))))
    root))

(defun dir-size (dir)
  (if dir
      (+ (sum (mapcar #'file-size (dir-files dir)))
         (sum (mapcar #'dir-size (dir-dirs dir))))
    0))

(defun find-small (dir)
  (if dir
      (+ (sum (mapcar #'find-small (dir-dirs dir)))
         (if (< (dir-size dir) 100000)
             (dir-size dir)
           0))
    0))

(defun part1 (file)
  (find-small (parse file)))
...
(defun dir-sizes (dir)
  (when dir
    (cons (dir-size dir)
          (mapcar #'dir-sizes (dir-dirs dir)))))

(defun part2 (file)
  (let* ((root (parse file))
         (target (- (dir-size root) 40000000)))
    (car
     (seq-drop-while
      (lambda (size) (< size target))
      (sort (flatten-list (dir-sizes root)) #'<)))))
