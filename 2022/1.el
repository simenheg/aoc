...
;; As your boats approach land, the Elves begin taking inventory of
;; their supplies. One important consideration is food - in
;; particular, the number of Calories each Elf is carrying (your
;; puzzle input).
...
(defun part1 (file)
  (let ((high 0)
    ...
    (dolist (line (slurp file))
      (cond
       ((equal line "")
        (setq high (max high sum))
        (setq sum 0))
       (t
        (cl-incf sum (string-to-number line)))))
    high))
...
(defun part2 (file)
  (let ((elves '(0)))
    (dolist (line (slurp file))
      (if (equal line "")
          (push 0 elves)
        (cl-incf (car elves) (string-to-number line))))
    (sum (seq-take (sort elves #'>) 3))))
