...
;;     A lone number means the monkey's job is simply to yell that
;;     number.
...
(defun part1 (file)
  (let ((values (make-hash-table :test #'equal))
    ...
    (dolist (line (slurp file))
      (pcase (split-string (string-replace ":" "" line))
        (`(,var ,x ,op ,y)
         (push `(,var ,x ,(intern op) ,y) Q))
        (`(,var ,val)
         (puthash var (string-to-number val) values))))
    (while-let ((next (pop Q)))
      (pcase-let ((`(,var ,x ,op ,y) next))
        (let ((x-val (gethash x values))
              (y-val (gethash y values)))
          (if (and x-val y-val)
              (puthash var (funcall op x-val y-val) values)
            (setq Q (append Q `(,next)))))))
    (gethash "root" values)))
...
;; Second, you got the wrong monkey for the job starting with
;; humn:. It isn't a monkey - it's you. Actually, you got the job
;; wrong, too: you need to figure out what number you need to yell so
;; that root's equality check passes. (The number that appears after
...
(defun eval-with (file humn-val root-x root-y)
  (let ((values (make-hash-table :test #'equal))
        (Q '()))
    (puthash "humn" humn-val values)
    (dolist (line (slurp file))
      (pcase (split-string (string-replace ":" "" line))
        (`(,var ,x ,op ,y)
         (push `(,var ,x ,(intern op) ,y) Q))
      ...
      (pcase-let ((`(,var ,x ,op ,y) next))
        (let ((x-val (gethash x values))
          ...
          (if (and x-val y-val)
              (puthash var (funcall op x-val y-val) values)
            (setq Q (append Q `(,next)))))))
    (- (gethash root-x values) (gethash root-y values))))

(defun part2 (file root-x root-y)
  (let ((lo 0) (hi 10000000000000))
    (catch 'done
      (while t
        (let* ((mid (/ (+ lo hi) 2))
               (val (eval-with file mid root-x root-y)))
          (cond ((> val 0) (setq lo mid))
                ((< val 0) (setq hi mid))
                (t (throw 'done mid))))))))
