...
(defun parse-board (raw-board)
  (let ((rows (make-hash-table))
        (cols (make-hash-table)))
    (seq-do-indexed
     (lambda (row row-n)
       (puthash row-n (make-hash-table) rows)
       (seq-do-indexed
        (lambda (char col-n)
          (unless (= char ?\s)
            (unless (gethash col-n cols)
              (puthash col-n (make-hash-table) cols))
            (puthash col-n (char-to-string char) (gethash row-n rows))
            (puthash row-n (char-to-string char) (gethash col-n cols))))
        row))
     (split-string raw-board "\n"))
    (list rows cols)))

(defun parse-instr (instr)
  (let ((res '()))
    (while (not (equal instr ""))
      (let ((next
             (seq-take-while
              (lambda (c) (string-match-p "[0-9]" (char-to-string c)))
              instr)))
        (push (string-to-number next) res)
        (setq instr (string-remove-prefix next instr))
        (unless (equal "" instr)
          (push (char-to-string (elt instr 0)) res)
          (setq instr (seq-drop instr 1)))))
    (reverse res)))

(defun parse (in)
  (pcase-let ((`(,raw-board ,raw-instr) (split-string in "\n\n")))
    (parse-board raw-board)
    (parse-instr (string-trim raw-instr))))
