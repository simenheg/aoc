...
(defun score (line)
  (seq-let (them us) (string-split line)
    (+
     (cond
      ((member line '("A X" "B Y" "C Z"))
       3)
      ((member line '("A Y" "B Z" "C X"))
       6)
      (t 0))
     (assoc-default us scores))))

(defun part1 (file)
  (sum (mapcar #'score (slurp file))))
...
(defun strat (line)
  (seq-let (them us) (string-split line)
    (string-join
     (list them
           (pcase them
             ("A" (pcase us ("X" "Z") ("Y" "X") ("Z" "Y")))
             ("B" (pcase us ("X" "X") ("Y" "Y") ("Z" "Z")))
             ("C" (pcase us ("X" "Y") ("Y" "Z") ("Z" "X")))))
     " ")))

(defun part2 (file)
  (sum (mapcar #'score (mapcar #'strat (slurp file)))))
