...
(defun score (winning hand)
  (let ((winners (seq-intersection winning hand)))
    (if winners
...
(defun part1 (file)
  (let ((sum 0))
    (pcase-dolist (`(_ ,winning ,hand)
                   (mapcar
                    (lambda (line) (string-split line "[:|]"))
                    (slurp file)))
      (cl-incf sum (score (string-split winning) (string-split hand))))
    sum))
...
(defun num-winners (winning hand)
  (length (seq-intersection winning hand)))

(defun part2 (file)
  (let* ((stack
          (mapcar
           (lambda (line) (mapcar #'string-split (cdr (string-split line "[:|]"))))
           (slurp file)))
         (copies (make-list (length stack) 1))
         (i 0))
    (dolist (line stack)
      (dotimes (x (apply #'num-winners line))
        (cl-incf (nth (+ x i 1) copies) (nth i copies)))
      (cl-incf i))
    (sum copies)))
