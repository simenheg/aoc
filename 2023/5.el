...
(defun map-it* (map-entry it)
  (pcase-let ((`(,dst ,src ,len) map-entry))
    (when (and (<= src it) (> (+ src len) it))
      (+ dst (- it src)))))

(defun map-it (map it)
  (or (seq-some
       (lambda (map-entry) (map-it* map-entry it))
       map)
      it))

(defun map-through (val maps)
  (dolist (map maps)
    (setq val (map-it map val)))
  val)

(defun minimize (seeds maps)
  (let ((min-loc (map-through (car seeds) maps)))
    (dolist (seed seeds)
      (let ((loc (map-through seed maps)))
        (setq min-loc (min min-loc loc))))
    min-loc))

(defun part1 (file)
  (let* ((in (slurp file))
         (seeds (mapcar #'string-to-number (cdr (string-split (car in)))))
         (maps '()))
    (dolist (line (cdr in))
      (cond
       ((string-match-p "^[a-z]" line) (push '() maps))
       ((string-match-p "^[0-9]" line)
        (push (mapcar #'string-to-number (string-split line))
              (car maps)))))
    (minimize seeds (reverse maps))))
...
(defun is-seed (num seeds)
  (seq-some
   (pcase-lambda (`(,from ,n))
     (and (>= num from) (< num (+ n from))))
   seeds))
...
(defun part2 (file)
  (let* ((in (slurp file))
         (seeds
          (seq-partition
           (mapcar #'string-to-number (cdr (string-split (car in))))
           2))
         (maps '())
         (min-pos nil)
         (i 0))
    (dolist (line (cdr in))
      (cond
       ((string-match-p "^[a-z]" line) (push '() maps))
       ((string-match-p "^[0-9]" line)
        (pcase-let ((`(,dst ,src ,len)
                     (mapcar #'string-to-number (string-split line))))
          (push `(,src ,dst ,len) (car maps))))))
    (while (not min-pos)
      (if (is-seed (map-through i maps) seeds)
          (setq min-pos i)
        (cl-incf i)))
    min-pos))
