...
;; (pcase-let ((`(_ ,id _ _ ,rate _ _ _ _ . ,to) (split-string lll)))
;;   `(,id
...
(defun parse-draws (draws)
  (mapcar
   (lambda (draw)
     (pcase-let ((`(,num ,color) (split-string draw " " t)))
       (list (string-to-number num) color)))
   (split-string draws ",")))

(defun parse-line (line)
  "line => (id draws)"
  (pcase-let ((`(,game ,draws) (split-string line ":")))
    (list
     (string-to-number (seq-drop game 5))
     (mapcar #'parse-draws (split-string draws ";")))))

(defun possible (id draws)
  "id draws => id | 0"
  (let ((highs (map-copy '(("red" . 0) ("green" . 0) ("blue" . 0)))))
    (dolist (draw draws)
      (pcase-dolist (`(,num ,color) draw)
        (map-put! highs color (max (map-elt highs color) num))))
    (if (and (>= 12 (map-elt highs "red"))
             (>= 13 (map-elt highs "green"))
             (>= 14 (map-elt highs "blue")))
        id
      0)))
...
(defun part1 (file)
  (sum
   (mapcar
    (lambda (line)
      (apply #'possible (parse-line line)))
    (slurp file))))
...
;;     In game 1, the game could have been played with as few as 4
;;     red, 2 green, and 6 blue cubes. If any color had even one fewer
...
(defun set-power (_id draws)
  (let ((highs (map-copy '(("red" . 0) ("green" . 0) ("blue" . 0)))))
    (dolist (draw draws)
      (pcase-dolist (`(,num ,color) draw)
        (map-put! highs color (max (map-elt highs color) num))))
    (* (map-elt highs "red") (map-elt highs "green") (map-elt highs "blue"))))

(defun part2 (file)
  (sum
   (mapcar
    (lambda (line)
      (apply #'set-power (parse-line line)))
    (slurp file))))
