...
(defun matchingp (c1 c2)
  (member (concat c1 c2) '("[]" "{}" "<>" "()")))

(defun parse-line (line &optional stack)
  (when line
    (let ((next (pop line)))
      (if (openingp next)
          (parse-line line (cons next stack))
        (let ((prev (pop stack)))
          (if (matchingp prev next)
              (parse-line line stack)
            next))))))

(defun score (c)
  (or (map-elt '((")" . 3) ("]" . 57) ("}" . 1197) (">" . 25137)) c)
      0))

(defun part1 (file)
  (sum (mapcar #'score (mapcar #'parse-line (parse file)))))
...
(defun score-incomplete (l)
  (let ((score 0))
    (dolist (c l)
      (setq score (* score 5))
      (cl-incf score (score2 c)))
    score))
...
(defun parse-line-2 (line &optional stack)
  (if line
      (let ((next (pop line)))
        (if (openingp next)
            (parse-line-2 line (cons next stack))
          (when (matchingp (pop stack) next)
            (parse-line-2 line stack))))
    stack))
...
(defun part2 (file)
  (let ((scores
         (mapcar
          ...
          (seq-filter
           #'identity
           (mapcar #'parse-line-2 (parse file))))))
