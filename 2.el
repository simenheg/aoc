...
(defun part1 (file)
  (let ((pos 0)
        (depth 0))
    (dolist (i (slurp file))
      (pcase-let ((`(,command ,amount) (split-string i)))
        (let ((n (string-to-number amount)))
          (pcase command
            ("forward" (cl-incf pos n))
            ("down" (cl-incf depth n))
            ("up" (cl-decf depth n))
            (t (error "Unknown command: %s" command))))))
    (list pos depth (* pos depth))))
